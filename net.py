...
X_train = np.asarray(X_train[permutation], dtype=np.float32).reshape((X_train.shape[0],32,32,3))
...
batch_size = 32 #using sgd, we'll use one batch of images at a time
...
def kaiming_normal(shape):
    ...
    return tf.random_normal(shape,stddev=0.7) * np.sqrt(2.0 / fan_in)

def forward(x,w1,w2,w3,w4,b1,b2,b3,b4):
    N = x.shape[0]
    layer = tf.nn.relu(tf.matmul(x, w1)+b1)
    layer = tf.nn.relu(tf.matmul(layer, w2)+b2)
    layer = tf.nn.relu(tf.matmul(layer, w3)+b3)
    layer = tf.nn.softmax(tf.matmul(layer, w4)+b4)
    return layer
def forward_drop(x,w1,w2,w3,w4,b1,b2,b3,b4):
    N = x.shape[0]
    layer = tf.nn.relu(tf.matmul(x, w1)+b1)
    layer = tf.nn.relu(tf.matmul(layer, w2)+b2)
    layer = tf.nn.relu(tf.matmul(layer, w3)+b3)
...
fcw3 = tf.Variable(kaiming_normal((hidden_size3, hidden_size4)))
fcw4 = tf.Variable(kaiming_normal((hidden_size4, num_labels)))

fcb3 = tf.Variable(tf.zeros((hidden_size4)))
fcb4 = tf.Variable(tf.zeros((num_labels)))

soft_scores = forward_drop(tf_train_data,fcw,fcw2,fcw3,fcw4,fcb,fcb2,fcb3,fcb4)


val_pred = forward(tf_val_data,fcw,fcw2,fcw3,fcw4,fcb,fcb2,fcb3,fcb4)

test_pred = forward(tf_test_data,fcw,fcw2,fcw3,fcw4,fcb,fcb2,fcb3,fcb4)
...
with tf.Session() as sess:
    ...
    for i in xrange(epochs):
        offset = 0
        while offset < X_train.shape[0]-batch_size:
            #offset = i*batch_size % (X_train.shape[0]-indent)
            #idx = np.random.randint(0,X_train.shape[0],indent)
            ...
            if g % print_every==0:
                ...
                print("train", train[-1])
                print "at global step: ",g,"lr = ", sess.run(learn)
            offset += indent
...
plt.plot(np.linspace(0,epochs,len(train)),train,label="train")
